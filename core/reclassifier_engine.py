...
class ReclassifierEngine:
    ...
    def draw_lines_from_table(self):
        ...
        if hasattr(self, "_interval_lines"):
            for line_artist in self._interval_lines:
                line_artist.remove()
        self._interval_lines = []
        ...
        elif all(hasattr(self.line_color, attr) for attr in ("red", "green", "blue")):
    ...
    def _on_click(self, event):
        ...
        if row in self._row_lines:
            previous_line = self._row_lines.pop(row)
            if getattr(previous_line, "axes", None) is not None:
        ...
        if fixed_x is not None:
            ...
            if row in self._fixed_lines:
                previous_fixed_line = self._fixed_lines.pop(row)
                if getattr(previous_fixed_line, "axes", None) is not None:
                    previous_fixed_line.remove()
